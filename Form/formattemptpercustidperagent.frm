...
'========asep================'
'    strsql = " select b.*,a.touch from ( " & vbCrLf
'    strsql = strsql & " select agent, count(agent) touch from mgm_hst where tgl between date(now()) and date(now()) group by 1 " & vbCrLf
'    strsql = strsql & " ) a,( " & vbCrLf
'    strsql = strsql & " select agent, count(custid) custid from ( " & vbCrLf
'    strsql = strsql & " select distinct(custid),agent from mgm_hst where tgl between date(now()) and date(now()) group by 1,2) a group by 1 " & vbCrLf
'    strsql = strsql & " ) b where a.agent = b.agent "
'============================'
