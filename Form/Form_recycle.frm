...
Begin VB.Form Form_recycle 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabsPerRow      =   2
      TabHeight       =   520
      TabCaption(0)   =   "Criteria Transfer"
      ...
      Begin Threed.SSFrame SSFrame1 
         ...
         Begin VB.CommandButton Command1 
            ...
            Width           =   1140
         End
         Begin VB.ComboBox cbotelesales2 
            Height          =   315
            Left            =   2145
            TabIndex        =   46
            Top             =   675
      ...
      Begin VB.Frame Frame3 
         BackColor       =   &H00FFFFFF&
         Caption         =   "Status call yang akan di Transfer"
         Height          =   840
         Left            =   8280
         TabIndex        =   2
         Top             =   405
...
Private Sub header_distribusi_Recycle()
    ...
    LVRecycle.ColumnHeaders.ADD 5, , "Jumlah yang di Transfer", 20 * TXT
    ...
    ListView1(1).ColumnHeaders.ADD 4, , "Agent Lama", 20 * TXT
    ListView1(1).ColumnHeaders.ADD 5, , "Agent Baru", 20 * TXT
    ListView1(1).ColumnHeaders.ADD 6, , "Jumlah data", 15 * TXT
...
Private Sub cbotelesales2_DropDown()
    load_telesales2
End Sub
...
ElseIf MDIForm1.txtlevel = "Supervisor" Then
    ...
    'mwhere = mwhere + "  agent in (select  userid  from usertbl where spvcode= '" + MDIForm1.TxtUsername.text + "' and  AKTIF ='1') "
End If
...
intvrl = InStr(1, cbosupervisor, "-", vbTextCompare)
    ...
    If MDIForm1.txtlevel = "Supervisor" Then
        ...
        'mwhere = mwhere & " agent in (select  userid  from usertbl where spvcode= '" + getUserid + "' and  AKTIF ='1') "
    ...
    Else
        ...
        'mwhere = mwhere & "  agent in (select  userid  from usertbl where (spvcode= '" + getUserid + "' or userid='" + getUserid + "'  ) and  AKTIF ='1') "
    End If
End If
...
If cbotelesales.text <> "" Then
    ...
    'mwhere = mwhere & "  agent = '" + getUserid + "' "
    '====asep===='
'Else
'    intvrl = InStr(1, cbotelesales, "-", vbTextCompare)
'    If intvrl <> 0 Then
'       ArrayString = Split(cbotelesales(1).text, "-", 2, vbTextCompare)
'       getUserid = ArrayString(0)
'       getUser_name = ArrayString(1)
'    End If
'    mwhere = mwhere + " and agent = '" + getUserid + "' "
...
sStrsql = "select distinct agent,nama_agent,count(distinct custid) as jmllead,'" + jumlahdata + "' as jmltransfer   from mgm where (statuscall<>'PTP' OR COALESCE(STATUSCALL,'')='') " & mwhere & " group by agent,nama_agent "
sstrSql2 = "select *  from mgm where (statuscall<>'PTP' OR COALESCE(STATUSCALL,'')='')  " & mwhere & "  "
...
While Not mobjrs2.EOF
     ...
     'list.SubItems(4) = IIf(IsNull(mobjrs2!jmltransfer), "", mobjrs2!jmltransfer)
...
If cmbcampaigncode.text <> "" Then
    mwhere = mwhere + " recsource = '" + cmbcampaigncode.text + "'"
End If
...
Select Case Index
    ...
    'STRSQL = " select tglentry,userinput ,count(tblrecyle_no_case) as jml ,agent from tblrecyle_hst group by tglentry,userinput,agent "
    STRSQL = "select a.* from(select tglentry,userinput ,count(tblrecyle_no_case) as jml ,agent,agent_lama from tblrecyle_hst group by tglentry,userinput,agent,agent_lama) a order by tglentry desc"
    ...
    While Not mobjrs.EOF
        ...
        list.SubItems(3) = IIf(IsNull(mobjrs!agent_lama), "0", mobjrs!agent_lama)
        list.SubItems(4) = IIf(IsNull(mobjrs!AGENT), "0", mobjrs!AGENT)
        list.SubItems(5) = IIf(IsNull(mobjrs!jml), "0", mobjrs!jml)
        mobjrs.MoveNext
    Wend
End Select
End Sub
...
Private Sub Command1_Click()
    Dim agentlama As String
    Dim agentbaru As String
    Dim jumlahdata As String
    
    agentlama = LVRecycle.SelectedItem.SubItems(1)
    agentbaru = cbotelesales2.text
    jumlahdata = TxtJumlah.text
    ...
    waktu = FungsiWaktuServer
    sstrSql2 = "select id  from mgm where   (STATUSCALL <> 'PTP' OR COALESCE(STATUSCALL,'')='')  " + mwhere + "  "

'    STRSQL = "insert into  tblrecyle_hst (recycle_id,tblrecyle_no_case,nama ,remarks,tglentry,agent,nmagent,statuscall ,campaign_code,userinput )"
'    STRSQL = STRSQL + " select id,CUSTID,name,remarks,'" + waktu + "',agent,nama_agent,statuscall,recsource,'" + MDIForm1.txtnama.text + "'"
'    STRSQL = STRSQL + " from mgm where ID in (" + sstrSql2 + " )"
'    STRSQL = STRSQL + " and agent='" + agentlama + "' limit " + jumlahdata + ""
    STRSQL = "insert into  tblrecyle_hst (recycle_id,tblrecyle_no_case,nama ,remarks,tglentry,agent_lama,agent,statuscall ,campaign_code,userinput )"
    STRSQL = STRSQL + " select id,CUSTID,name,remarks,'" + waktu + "','" + agentlama + "','" + agentbaru + "',statuscall,recsource,'" + MDIForm1.txtnama.text + "'"
    STRSQL = STRSQL + " from mgm where ID in (" + sstrSql2 + " )"
    STRSQL = STRSQL + " and agent='" + agentlama + "' limit " + jumlahdata + ""
    M_OBJCONN.Execute (STRSQL)
    
    CMDSQL = "update mgm set agent = '" + agentbaru + "' where custid in (select distinct custid from mgm where agent='" + agentlama + "' and (statuscall<>'PTP' OR COALESCE(STATUSCALL,'')='')" & mwhere & "  limit " + jumlahdata + ")"
    M_OBJCONN.Execute CMDSQL
    MsgBox "Transfer data dari agent " + agentlama + " ke " + agentbaru + " sejumlah = " + jumlahdata + " berhasil!"
    
    LVRecycle.SelectedItem.SubItems(4) = jumlahdata
    txtSisaCampaign.text = LVRecycle.SelectedItem.SubItems(3) - jumlahdata
    txtSudahDistribusi.text = jumlahdata
...
'        M_objrs.CursorLocation = adUseClient
'        M_objrs.Open sStrsql, M_OBJCONN, adOpenDynamic, adLockOptimistic
'       cbotelesales(1).clear
'        While Not M_objrs.EOF
'                cbotelesales(1).AddItem IIf(IsNull(M_objrs!USERID), "", M_objrs!USERID) & "-" & IIf(IsNull(M_objrs!AGENT), "", M_objrs!AGENT)
'                M_objrs.MoveNext
'        Wend
'    Set M_objrs = Nothing
'    End Select
End Sub
Public Sub load_telesales2()
Dim getUserid As String
Dim getUser_name As String
    If MDIForm1.txtlevel.text = "Agent" Then
        sStrsql = " select  userid , agent  from usertbl where  userid = '" + MDIForm1.TxtUsername.text + "' and AKTIF ='1'"
    ElseIf MDIForm1.txtlevel.text = "Supervisor" Then
        sStrsql = " select  userid , agent  from usertbl where spvcode = '" + MDIForm1.TxtUsername.text + "' and  AKTIF ='1' and agent not in ('" & LVRecycle.SelectedItem.SubItems(1) & "')"
    Else
        intvrl = InStr(1, cbosupervisor, "-", vbTextCompare)
           If intvrl <> 0 Then
              ArrayString = Split(cbosupervisor.text, "-", 2, vbTextCompare)
    ...
    End If
        ...
        Set M_objrs = New ADODB.Recordset
           ...
           cbotelesales2.clear
            While Not M_objrs.EOF
                    'cbotelesales2.AddItem IIf(IsNull(M_objrs!USERID), "", M_objrs!USERID) & "-" & IIf(IsNull(M_objrs!AGENT), "", M_objrs!AGENT)
                    cbotelesales2.AddItem IIf(IsNull(M_objrs!USERID), "", M_objrs!USERID)
                    M_objrs.MoveNext
            Wend
        Set M_objrs = Nothing
End Sub
...
End If
    ...
    setJmlDistribusi = TxtJumlah.text
...
'            MsgBox "Maaf Data Yang Di masukan Harus Angka / Numeric...!", vbInformation
'        Exit Sub
'        End If
'    End If
'End Sub
