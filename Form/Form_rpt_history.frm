...
Begin VB.Form Form_rpt_history 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdCari 
         Height          =   375
         Left            =   6525
         Picture         =   "Form_rpt_history.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   12
         Top             =   825
         Width           =   1605
      ...
      Begin VB.CommandButton SSCommand1 
         ...
         Height          =   1020
         Left            =   6525
         Picture         =   "Form_rpt_history.frx":05EE
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   1245
         Width           =   1620
      ...
      Begin VB.CommandButton SSCommand2 
         ...
         Height          =   360
         Left            =   6525
         Picture         =   "Form_rpt_history.frx":0D54
         Style           =   1  'Graphical
         TabIndex        =   1
         Top             =   2340
         Width           =   1620
      ...
      Begin TDBDate6Ctl.TDBDate tgl_call 
         ...
         _ExtentX        =   2408
         _ExtentY        =   556
         Calendar        =   "Form_rpt_history.frx":139A
         Caption         =   "Form_rpt_history.frx":14B2
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "Form_rpt_history.frx":151E
         Keys            =   "Form_rpt_history.frx":153C
         Spin            =   "Form_rpt_history.frx":159A
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
      ...
      Begin TDBDate6Ctl.TDBDate tgl_call 
         ...
         _ExtentX        =   2302
         _ExtentY        =   556
         Calendar        =   "Form_rpt_history.frx":15C2
         Caption         =   "Form_rpt_history.frx":16DA
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "Form_rpt_history.frx":1746
         Keys            =   "Form_rpt_history.frx":1764
         Spin            =   "Form_rpt_history.frx":17C2
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
         BackColor       =   -2147483643
   ...
   End
End
Attribute VB_Name = "Form_rpt_history"
...
Dim MOBJ As ADODB.Recordset
    ...
    STRSQL = "select * from (select agent as ""Agent"",status_break as ""StatusBreak"",date(date_break)""Date Break"", sum(waktu_end - waktu_start) as ""Durasi"" from tbl_autodialer_agent_break"
    STRSQL = STRSQL + " where status_break ='Meeting' and agent in (" + ListCustId + ")"
    STRSQL = STRSQL + " and date(date_break) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "'group by agent,status_break,date(date_break) Union All"
    STRSQL = STRSQL + " select agent,status_break,date(date_break), sum(waktu_end - waktu_start)as pray from tbl_autodialer_agent_break where status_break ='Pray' and agent in (" + ListCustId + ")"
    STRSQL = STRSQL + " and date(date_break) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "' group by agent,status_break,date(date_break) Union All"
    STRSQL = STRSQL + " select agent,status_break,date(date_break), sum(waktu_end - waktu_start) as toilet from tbl_autodialer_agent_break where status_break ='Toilet' and agent in (" + ListCustId + ") "
    STRSQL = STRSQL + " and date(date_break) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "' group by agent,status_break,date(date_break) Union All"
    STRSQL = STRSQL + " select agent,status_break,date(date_break), sum(waktu_end - waktu_start)as lunch from tbl_autodialer_agent_break where status_break ='Lunch' and agent in (" + ListCustId + ")"
    STRSQL = STRSQL + " and date(date_break) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "' group by agent,status_break,date(date_break) Union All"
    STRSQL = STRSQL + " select agent,status_break,date(date_break), sum(waktu_end - waktu_start) as coaching from tbl_autodialer_agent_break where status_break ='Coaching' and agent in (" + ListCustId + ") "
    STRSQL = STRSQL + " and date(date_break) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "' group by agent,status_break,date(date_break) ) a order by  3,2"
