...
Dim OBJRECORD As ADODB.Recordset
    ...
    CMDSQL = "select * from  mandiri.tbldivisi where    nm_divisi='"
...
Dim OBJRECORD As ADODB.Recordset
    ...
    'Mengisi data ke combo campaigncode
    CMDSQL = "select * from  mandiri.tbldivisi where kddivisi='"
       ...
       M_OBJCONN.Execute "delete from tbl_temp_field_payment "
        If M_objrs.EOF And M_objrs.BOF Then Exit Sub
            For i = 0 To M_objrs.fields.Count - 1
                ...
                STRSQL = "insert into mandiri.tbl_temp_field_payment (nama_field) values ('" + M_objrs.fields(i).Name + "')"
...
Public Sub loadCboMap()
    cbomap.CLEAR
    ssql = "select DISTINCT(kode_source) from mandiri.tbl_setting_upload_cpa  where (kode_source is not null or kode_source<>'')"
    Set M_objrs = New ADODB.Recordset
        M_objrs.CursorLocation = adUseClient
...
Dim list As ListItem
    ...
    sStrsql = sStrsql + "  select field_source,field_destination from mandiri.tbl_setting_upload_cpa where kode_source='" + xCodeMap + "' ) "
...
Private Sub cmdProses_Click()
 ...
 Dim mobjtemp As New ADODB.Recordset
   ...
   If cekMANDATORTY = False Then
    ...
    mobjtemp.Open "select * from mandiri.tbl_upload_temp_payment", M_OBJCONN, adOpenDynamic, adLockOptimistic
...
Set M_objrs = New ADODB.Recordset
    ...
    STRSQL = "  select " + strinsert + " from  mandiri.tbl_upload_temp_payment where (F_FLAG IS NULL OR F_FLAG=0) "
...
njmlNew = M_objrs.RecordCount
  If MsgBox("New Data :" + CStr(njmlNew) + vbCrLf + "", vbQuestion + vbYesNo, "Question") = vbYes Then
    If strinsert <> "" Then
        STRSQL = "insert into mandiri.tbllunas (" + strinsert + ")"
        STRSQL = STRSQL + "  select " + strinsert + " from  mandiri.tbl_upload_temp_payment where (F_FLAG IS NULL OR F_FLAG=0) "
        M_OBJCONN.Execute (STRSQL)
     ...
     STRSQL = "insert into mandiri.tbl_hst_upload (userid,nama,location_file,Sheet,lead,eksekusi,jml_row) values ("
...
Dim list As ListItem
    ...
    sStrsql = sStrsql + " full join  ( "
    sStrsql = sStrsql + "  select field_source,field_destination from mandiri.tbl_setting_upload where substring(field_source,1,2) in ('n_','v_','d_') and kode_source='" + xCodeMap + "' ) "
    sStrsql = sStrsql + " as tbldua on tblbaru.nama_kolom =tbldua.field_source) as tblbaru "
...
Public Function cekmapping_excel() As Boolean
    ...
    STRSQL = STRSQL + " (   select field_source,field_destination from mandiri.tbl_setting_upload_cpa  where kode_source='" + cbomap.Text + "')"
    ...
    STRSQL = STRSQL + " Left Join ( select * from mandiri.tbl_temp_field_cpa  ) as tbldua   on tblsatu.field_destination=tbldua.nama_field"
    Set M_objrs = New ADODB.Recordset
        M_objrs.CursorLocation = adUseClient
        M_objrs.Open STRSQL, M_OBJCONN, adOpenDynamic, adLockOptimistic
       If M_objrs.RecordCount = 0 Then
...
Dim list As ListItem
Dim no As Double
sStrsql = "select * from mandiri.tbl_hst_upload "
Set M_objrs = New ADODB.Recordset
    M_objrs.CursorLocation = adUseClient
    M_objrs.Open sStrsql, M_OBJCONN, adOpenDynamic, adLockOptimistic
    no = 0
...
On Error Resume Next
 ...
 STRSQL = STRSQL + " WHERE table_name='tbllunas' and data_type in ('character varying','numeric','bigint','integer','timestamp without time zone','text')    ORDER BY ordinal_position) as tblbaru  full join  (   select field_source,field_destination from mandiri.tbl_setting_upload_payment where  kode_source='" + cbomap.Text + "' ) "
 ...
 Set M_objrs = New ADODB.Recordset
   ...
   ProgressBar1.Max = M_objrs.RecordCount + 1
    ...
    ProgressBar1.Value = M_objrs.Bookmark
            ...
            If Trim(data_type) = "character varying" Then
                If data_length = "" Then
                    ...
                    sStrsql = " alter table mandiri.Tbl_Upload_Temp_payment  add column  " + Strtype
                Else
    ...
    Wend
        ...
        sStrsql = " alter table mandiri.Tbl_Upload_Temp_payment  add column f_flag numeric"
        M_OBJCONN.Execute (sStrsql)
...
'    MsgBox "sdsd"
    'End If
           ...
           If CEKIN = False Then
                ...
                If strfield <> "" Then
                        ssqlhead = "INSERT INTO mandiri.Tbl_Upload_Temp_payment (" + strfield + ") values ( " + strvalues + ")"
...
'
            ...
            STRSQL = " update   mandiri.tbl_upload_temp_payment set  f_flag=1 where id in (select Tbl_Upload_Temp_payment.id  from mandiri.tbllunas inner join "
            STRSQL = STRSQL + " mandiri.Tbl_Upload_Temp_payment  on tbllunas.custid= Tbl_Upload_Temp_payment.custid  and date(tbllunas.paydate)=date(Tbl_Upload_Temp_payment.paydate) AND tbllunas.payment=Tbl_Upload_Temp_payment.payment)"
            M_OBJCONN.Execute (STRSQL)
'
...
'            M_OBJCONN.Execute (STRSQL)
    ...
    'cek existing data
     ...
     STRSQL = " select * from mandiri.tbl_upload_temp_payment  where f_flag=1"
    ...
    STRSQL = " select custid,paydate,agent,sum(payment) as payment from mandiri.tbl_upload_temp_payment where f_flag is null or f_flag=0  group by custid,paydate,agent "
