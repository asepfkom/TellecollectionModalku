...
Begin VB.Form Frm_Upload_address 
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Upload Detail Customer"
   ClientHeight    =   10050
   ClientLeft      =   45
   ClientTop       =   315
   ClientWidth     =   17280
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox cbomap 
         Height          =   315
         ItemData        =   "Frm_Upload_address.frx":0000
         Left            =   1380
         List            =   "Frm_Upload_address.frx":0002
         TabIndex        =   15
         Top             =   300
         Width           =   2595
      End
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "Frm_Upload_address.frx":0004
      ...
      TabPicture(1)   =   "Frm_Upload_address.frx":0020
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "lstmapping"
      Tab(1).ControlCount=   1
      TabCaption(2)   =   "History Upload      "
      TabPicture(2)   =   "Frm_Upload_address.frx":003C
      Tab(2).ControlEnabled=   0   'False
      Tab(2).Control(0)=   "txtnumrowshst"
      Tab(2).Control(1)=   "cmdrefresh_hst"
      Tab(2).Control(2)=   "lsthst"
      Tab(2).Control(3)=   "Label11"
      Tab(2).ControlCount=   4
      TabCaption(3)   =   "Error In Excel        "
      TabPicture(3)   =   "Frm_Upload_address.frx":0058
      ...
      TabPicture(4)   =   "Frm_Upload_address.frx":0074
      ...
      Begin VB.Frame Frame6 
         ...
         Begin TabDlg.SSTab SSTab2 
            ...
            TabPicture(0)   =   "Frm_Upload_address.frx":0090
            Tab(0).ControlEnabled=   0   'False
            Tab(0).Control(0)=   "Txtxadarow"
            Tab(0).Control(1)=   "CmdCekAll"
            Tab(0).Control(2)=   "CndUncekAll"
            Tab(0).Control(3)=   "ListView1"
            Tab(0).Control(4)=   "Label10"
            Tab(0).ControlCount=   5
            TabCaption(1)   =   "History"
            TabPicture(1)   =   "Frm_Upload_address.frx":00AC
   ...
   Begin VB.Image Image1 
      ...
      Index           =   0
      Left            =   30
      Picture         =   "Frm_Upload_address.frx":00C8
      Stretch         =   -1  'True
      Top             =   30
      Width           =   420
   End
   Begin VB.Image Image2 
      Height          =   465
      Left            =   0
      Picture         =   "Frm_Upload_address.frx":0BD2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   17220
   End
End
Attribute VB_Name = "Frm_Upload_address"
...
Private Sub cmdcreatemap_Click()
   Form_setting_upload_address.Show 1
End Sub
Public Sub loadCboMap()
    cbomap.CLEAR
    ssql = "select DISTINCT(kode_source) from tbl_setting_upload_address  where (kode_source is not null or kode_source<>'')"
    Set M_objrs = New ADODB.Recordset
        M_objrs.CursorLocation = adUseClient
...
Public Sub findFx(ByVal xCodeMap)
    ...
    sStrsql = sStrsql + " SELECT column_name as nama_kolom,character_maximum_length,data_type From information_schema.Columns WHERE table_name='tbl_address' and table_schema='public'"
    ...
    sStrsql = sStrsql + "  select field_source,field_destination from tbl_setting_upload_address where kode_source='" + xCodeMap + "' ) "
...
strQuery = " select * from Tbl_Upload_Temp_Address where custid in (" + strdel + ")"
...
cek_record.Open "select distinct table_name from information_schema.columns where table_catalog='crm' and table_schema='public' and table_name ='" + NmFile + "'", M_OBJCONN, adOpenDynamic, adLockOptimistic
...
M_OBJCONN.Execute ("delete from Tbl_Upload_Temp_Address where custid in (" + strdel + ")")
...
'        MsgBox "Cek field mandatory harus tersedia custid!", vbOKOnly, "Information"
'        Exit Sub
'    End If
    ...
    mobjtemp.Open "select * from Tbl_Upload_Temp_Address WHERE (CUSTID IS NOT NULL OR CUSTID<>'' )", M_OBJCONN, adOpenDynamic, adLockOptimistic
  ...
  For jRow = 1 To lstview.ListItems.Count
        If Len(lstview.ListItems(jRow).SubItems(1)) > 0 Then
                If ncount = 1 Then
                    ...
                    strfieldheaderupdate = "Tbl_Upload_Temp_Address." + lstview.ListItems(jRow).Text + ",Tbl_Upload_Temp_Address.recsource"
                ...
                Else
                    ...
                    strfieldheaderupdate = strfieldheaderupdate + ",Tbl_Upload_Temp_Address." + lstview.ListItems(jRow).Text
...
Set M_objrs = New ADODB.Recordset
    M_objrs.CursorLocation = adUseClient
      strsql = " select  tbl_address.* from tbl_address,Tbl_Upload_Temp_Address "
      strsql = strsql + " where tbl_address.custid=Tbl_Upload_Temp_Address.custid  and  (Tbl_Upload_Temp_Address.f_flag=1 or Tbl_Upload_Temp_Address.f_flag is null"
      strsql = strsql + " or Tbl_Upload_Temp_Address.f_flag=0 )"
...
Set M_objrs = New ADODB.Recordset
    ...
    strsql = "  select " + strinsert + " from  Tbl_Upload_Temp_Address where id not in"
    strsql = strsql + " ( "
    strsql = strsql + " select  Tbl_Upload_Temp_Address.id from tbl_address,Tbl_Upload_Temp_Address"
    strsql = strsql + " where tbl_address.custid=Tbl_Upload_Temp_Address.custid ) and (CUSTID IS NOT NULL OR CUSTID <>'') AND "
    strsql = strsql + " (Tbl_Upload_Temp_Address.f_flag=1 or Tbl_Upload_Temp_Address.f_flag is null or Tbl_Upload_Temp_Address.f_flag=0 )   and  (f_flag_donot is null or f_flag_donot=0)"
    strsql = strsql + " AND (coalesce(contact1,'')<>''or coalesce(mobileno,'')<>'' or coalesce(contact2,'')<>'')"
...
njmlNew = M_objrs.RecordCount
  If MsgBox("New Data :" + CStr(njmlNew) + vbCrLf + "", vbQuestion + vbYesNo, "Question") = vbYes Then
    If strinsert <> "" Then
        strsql = "insert into tbl_address (" + strinsert + ")"
        strsql = strsql + "  select " + strinsert + " from  Tbl_Upload_Temp_Address where id not in"
        strsql = strsql + " ( "
        strsql = strsql + " select  Tbl_Upload_Temp_Address.id from tbl_address,Tbl_Upload_Temp_Address"
        strsql = strsql + " where tbl_address.custid=Tbl_Upload_Temp_Address.custid ) and "
        strsql = strsql + " (Tbl_Upload_Temp_Address.f_flag=1 or Tbl_Upload_Temp_Address.f_flag is null or Tbl_Upload_Temp_Address.f_flag=0 )   and  (f_flag_donot is null or f_flag_donot=0) AND Tbl_Upload_Temp_Address.CUSTID IS NOT NULL"
        strsql = strsql + " AND (coalesce(contact1,'')<>''or coalesce(mobileno,'')<>'' or coalesce(contact2,'')<>'')"
        
        M_OBJCONN.Execute (strsql)
        
        ' TAMBAHAN Field Class -> 835 di buat otomatis batch decease 28 NOP 2013
        'M_OBJCONN.Execute "UPDATE mgm SET RECSOURCE=RECSOURCE||'deceased' WHERE custid IN (SELECT custid FROM Tbl_Upload_Temp_Address WHERE cust_class='835'); "
        ' ----------------------------------------------------------------------
        ...
        list.SubItems(6) = "Insert New Data Address"
     ...
     strsql = strsql + "'" + Replace(Replace(cbosheet.Text, "$", ""), "'", "") + "'," + CStr(Val(txtrowssource.Text)) + ",'Insert New Data'," + CStr(Val(njmlNew)) + ",'ADDRESS')"
     M_OBJCONN.Execute (strsql)
    
    strsql = "insert into tbl_dontlist (appid,adr_type,email,homeno,mobileno,officeno,region,recsource,custid)"
    strsql = strsql + " select appid,adr_type,email,contact1,mobileno,contact2,city,recsource,custid from Tbl_Upload_Temp_Address where (coalesce(contact1,'')='' AND coalesce(mobileno,'')='' AND coalesce(contact2,'')='')"
    M_OBJCONN.Execute (strsql)
    strsql = "update tbl_address set mobileno =trim(mobileno),contact1=trim(contact1),contact2=trim(contact2)"
    M_OBJCONN.Execute (strsql)
    End If
End If
End If
...
'STRSQL = "SELECT DISTINCT(RECSOURCE), '" + TXTDESCRIPTION_BATCH + "' as batch  FROM Tbl_Upload_Temp_Address "


strsql = "SELECT DISTINCT(RECSOURCE), '" + TXTDESCRIPTION_BATCH + "' as batch  FROM Tbl_Upload_Temp_Address where recsource not in (select kodeds from datasourcetbl) "
...
Dim list As ListItem
    ...
    sStrsql = sStrsql + " select * from ( "
    sStrsql = sStrsql + " SELECT column_name as nama_kolom From information_schema.Columns WHERE table_name='tbl_address'"
    sStrsql = sStrsql + " and substring(column_name,1,2) in ('n_','v_','d_') ORDER BY ordinal_position) as tblbaru "
    sStrsql = sStrsql + " full join  ( "
    sStrsql = sStrsql + "  select field_source,field_destination from tbl_setting_upload_address where substring(field_source,1,2) in ('n_','v_','d_') and kode_source='" + xCodeMap + "' ) "
    sStrsql = sStrsql + " as tbldua on tblbaru.nama_kolom =tbldua.field_source) as tblbaru "
...
Public Function cekmapping_excel() As Boolean
    ...
    strsql = strsql + " SELECT column_name as nama_kolom From information_schema.Columns WHERE table_name='tbl_address'"
    ...
    strsql = strsql + " (   select field_source,field_destination from tbl_setting_upload_address  where kode_source='" + cbomap.Text + "')"
...
Dim list As ListItem
Dim no As Double
sStrsql = "select * from tbl_hst_upload WHERE jenis_upload='ADDRESS'"
Set M_objrs = New ADODB.Recordset
    M_objrs.CursorLocation = adUseClient
    M_objrs.Open sStrsql, M_OBJCONN, adOpenDynamic, adLockOptimistic
    no = 0
...
Dim m_existing_new  As ADODB.Recordset
 ...
 On Error Resume Next
 M_OBJCONN.Execute " Drop TABLE Tbl_Upload_Temp_Address "
 ssql = "CREATE TABLE Tbl_Upload_Temp_Address "
 ssql = ssql & "(id serial)"
 ...
 strsql = strsql + " WHERE table_name='tbl_address' and table_schema='public' and data_type in ('character varying','numeric','bigint','integer','timestamp without time zone','text') and column_name not in (select nama_kolom  from tbl_tidak_map)   ORDER BY ordinal_position) as tblbaru  full join  (   select field_source,field_destination from tbl_setting_upload_address where  kode_source='" + cbomap.Text + "' ) "
 strsql = strsql + "  as tbldua on tblbaru.nama_kolom =tbldua.field_source) as tblbaru"
 strsql = strsql + " Left Join"
 strsql = strsql + " (SELECT column_name,data_type ,character_maximum_length From information_schema.Columns WHERE table_name='tbl_address' and table_schema='public' ORDER BY ordinal_position) as tbltiga"
 ...
 Set M_objrs = New ADODB.Recordset
   ...
   ProgressBar1.Max = M_objrs.RecordCount + 1
            ...
            If Trim(data_type) = "character varying" Then
                If data_length = "" Then
                    If nama_kol = "limit" Then
                        ...
                        sStrsql = " alter table Tbl_Upload_Temp_Address  add column  " + Strtype
                    Else
    ...
    sStrsql = " alter table Tbl_Upload_Temp_Address  add column recsource character varying(255)"
    M_OBJCONN.Execute (sStrsql)
    
    
    
    
    sStrsql = " alter table Tbl_Upload_Temp_Address  add column f_flag numeric"
    M_OBJCONN.Execute (sStrsql)
    
    sStrsql = " alter table Tbl_Upload_Temp_Address  add column f_flag_donot numeric"
    M_OBJCONN.Execute (sStrsql)
...
'    MsgBox "sdsd"
    'End If
           ...
           If CEKIN = False Then
                ...
                If strfield <> "" Then
                        ssqlhead = "INSERT INTO Tbl_Upload_Temp_Address (" + strfield + ") values ( " + strvalues + ")"
    ...
    Set M_objrs = Nothing
           ...
           ssql = " update Tbl_Upload_Temp_Address set name=replace(name,'''','') where name like '%''%' "
            M_OBJCONN.Execute (ssql)
           ssql = " update Tbl_Upload_Temp_Address set name=replace(name,'.','') where name like '%.%'"
            M_OBJCONN.Execute (ssql)
             ssql = " update Tbl_Upload_Temp_Address set name=replace(name,'*','') where name like '%*%'"
            ...
            ssql = "  update Tbl_Upload_Temp_Address name=replace(name,'/','') where name like '%/%'"
            M_OBJCONN.Execute (ssql)
            ...
            ssql = " update Tbl_Upload_Temp_Address set  contact1 =substring(contact1,4,(length(contact1)-3)) where substring(contact1,1,3)='021'"
            M_OBJCONN.Execute (ssql)
            ssql = " update Tbl_Upload_Temp_Address set contact2 =substring(contact2,4,(length(contact2)-3)) where substring(contact2,1,3)='021'"
            M_OBJCONN.Execute (ssql)
           ssql = " update Tbl_Upload_Temp_Address set mobileno =substring(mobileno,4,(length(mobileno)-3)) where substring(mobileno,1,3)='021'"
            M_OBJCONN.Execute (ssql)
     
           ssql = " update Tbl_Upload_Temp_Address set mobileno2 =substring(mobileno2,4,(length(mobileno2)-3)) where substring(mobileno2,1,3)='021'; "
           M_OBJCONN.Execute (ssql)
           ssql = " update Tbl_Upload_Temp_Address set mobilenoadd1 =substring(mobilenoadd1,4,(length(mobilenoadd1)-3)) where substring(mobilenoadd1,1,3)='021';"
            M_OBJCONN.Execute (ssql)
            ssql = "  update Tbl_Upload_Temp_Address set mobilenoadd2 =substring(mobilenoadd2,4,(length(mobilenoadd2)-3)) where substring(mobilenoadd2,1,3)='021';"
            M_OBJCONN.Execute (ssql)
            
            ssql = " UPDATE Tbl_Upload_Temp_Address SET mobileno='0'||substr(trim(mobileno),3) WHERE mobileno like '62%' AND length(mobileno)>10"
            M_OBJCONN.Execute (ssql)

            strsql = " update  Tbl_Upload_Temp_Address set  f_flag=2 where id  not in (select min(id) from Tbl_Upload_Temp_Address group by v_no_kartu,v_nama,d_dob) "
            M_OBJCONN.Execute (strsql)
    
'            STRSQL = " update Tbl_Upload_Temp_Address set f_flag_donot =1 where v_no_kartu in (select no_kartu from tbldonotcall ) "
'            M_OBJCONN.Execute (STRSQL)
...
'    cobadeh.Open "select * from tbl_address ", M_OBJCONN, adOpenDynamic, adLockOptimistic, adCmdText
'
'    MsgBox cobadeh.RecordCount
    
    
    'cek existing data
      strsql = " select tbl_address.custid as unik ,tbl_address.* from tbl_address,Tbl_Upload_Temp_Address "
      strsql = strsql + " where tbl_address.custid=Tbl_Upload_Temp_Address.custid and (Tbl_Upload_Temp_Address.f_flag=1 or Tbl_Upload_Temp_Address.f_flag is null "
      strsql = strsql + " or Tbl_Upload_Temp_Address.f_flag=0)"
      strsql = strsql + " and (f_flag_donot is null or f_flag_donot=0) "
'STRSQL = "select * from tbl_address"
    ...
    'cek data new
      strsql = "  select * from  Tbl_Upload_Temp_Address where id not in"
      strsql = strsql + " ( "
      strsql = strsql + " select  Tbl_Upload_Temp_Address.id from tbl_address,Tbl_Upload_Temp_Address"
      strsql = strsql + " where tbl_address.custid=Tbl_Upload_Temp_Address.custid ) and (CUSTID IS NOT NULL OR CUSTID<>'') AND   "
      strsql = strsql + " (Tbl_Upload_Temp_Address.f_flag=1 or Tbl_Upload_Temp_Address.f_flag is null or Tbl_Upload_Temp_Address.f_flag=0  )   and  (f_flag_donot is null or f_flag_donot=0)"
    ...
    'cek jumlah data yang ke donot call
     'strsql = " select * from  Tbl_Upload_Temp_Address  where f_flag_donot =1 "
     strsql = "select name,addrnow,contact1,mobileno,addrpt,contact2,nocard,region,dob,recsource,custid,curbal,pay_dt,lastpay from Tbl_Upload_Temp_Address where (coalesce(contact1,'')='' AND coalesce(mobileno,'')='' AND coalesce(contact2,'')='')"
    Set m_objdonot = New ADODB.Recordset
        ...
        strsql = " select * from  Tbl_Upload_Temp_Address  where f_flag=2 AND  (f_flag_donot IS NULL OR f_flag_donot=0)  "
