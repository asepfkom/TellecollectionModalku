...
Begin VB.Form Form_Report_call_detail 
   BackColor       =   &H00C0C0C0&
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdCari 
         Height          =   375
         Left            =   6810
         Picture         =   "Form_Report_detailcall.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   18
         Top             =   1230
         Width           =   1605
      ...
      Begin VB.CommandButton SSCommand1 
         ...
         Height          =   1020
         Left            =   6795
         Picture         =   "Form_Report_detailcall.frx":05EE
         Style           =   1  'Graphical
         TabIndex        =   3
         Top             =   1680
         Width           =   1620
      ...
      Begin VB.CommandButton SSCommand2 
         ...
         Height          =   360
         Left            =   6780
         Picture         =   "Form_Report_detailcall.frx":0D54
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   2760
         Width           =   1620
      ...
      Begin TDBDate6Ctl.TDBDate tgl_call 
         ...
         _ExtentX        =   2408
         _ExtentY        =   556
         Calendar        =   "Form_Report_detailcall.frx":139A
         Caption         =   "Form_Report_detailcall.frx":14B2
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "Form_Report_detailcall.frx":151E
         Keys            =   "Form_Report_detailcall.frx":153C
         Spin            =   "Form_Report_detailcall.frx":159A
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
      ...
      Begin TDBDate6Ctl.TDBDate tgl_call 
         ...
         _ExtentX        =   2302
         _ExtentY        =   556
         Calendar        =   "Form_Report_detailcall.frx":15C2
         Caption         =   "Form_Report_detailcall.frx":16DA
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
            ...
            Strikethrough   =   0   'False
         EndProperty
         DropDown        =   "Form_Report_detailcall.frx":1746
         Keys            =   "Form_Report_detailcall.frx":1764
         Spin            =   "Form_Report_detailcall.frx":17C2
         AlignHorizontal =   0
         AlignVertical   =   0
         Appearance      =   1
         BackColor       =   -2147483643
   ...
   Begin VB.Image Image2 
      Height          =   825
      Left            =   0
      Picture         =   "Form_Report_detailcall.frx":17EA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   19245
   End
End
Attribute VB_Name = "Form_Report_call_detail"
...
Dim MOBJ As ADODB.Recordset
    ...
    strsql = " select date(tglcall)::varchar as ""Tgl Call"",total_Call1 as ""Total Call"",jumlah_polis as ""Jumlah Polis"" --,jml_newdata as ""New Data"""
    ...
    strsql = strsql + vbCrLf + " (select date(tgl) as tgl, count(date(tgl)) as total_Call1 from mgm_hst where custid in(select custid from mgm where agent in (" + ListCustId + ") AND date(tglcall) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "'  ) "
    strsql = strsql + vbCrLf + " and agent in (" + ListCustId + ") AND date(tgl) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "' group by date(tgl) order by date(tgl) "
    strsql = strsql + vbCrLf + " )y on x.tglcall=y.tgl"
    strsql = strsql + vbCrLf + " left join"
    strsql = strsql + vbCrLf + " (select date(tglcall) as tglcall1, count(date(tglcall)) as jumlah_polis from mgm where statuscall <> '' and agent in (" + ListCustId + ") AND date(tglcall) between '" + Format(tgl_call(0).Value, "yyyy-mm-dd") + "' and '" + Format(tgl_call(1).Value, "yyyy-mm-dd") + "'    group by date(tglcall) order by date(tglcall)"
    strsql = strsql + vbCrLf + " )z on x.tglcall=z.tglcall1"
    strsql = strsql + vbCrLf + " UNION ALL"
    strsql = strsql + vbCrLf + " select 'TOTAL',sum(""Total Call""),sum(""Jumlah Polis"") --,jml_newdata as ""New Data"""
    strsql = strsql + vbCrLf + " ,sum(""Already Paid""),sum(""BP""),sum(""PTP""),sum(""Schedule Call""),sum(""Left Message""),sum(""Negosiasi""),sum(""Busy""),sum(""Dead""),sum(""Invalid""),sum(""Mailbox""),sum(""Pindah Alamat""),sum(""Salah Sambung""),sum(""Tidak Ada di Tempat""),sum(""Tidak Diangkat""),sum(""Unknow""),sum(""Data Retur""),sum(""Nominal PTP""),sum(""Tagihan"") from ("
    strsql = strsql + vbCrLf + " select date(tglcall) as ""Tgl Call"",total_Call1 as ""Total Call"",jumlah_polis as ""Jumlah Polis"""
    strsql = strsql + vbCrLf + " ,jml_paid as ""Already Paid"",jml_bp as ""BP"",jml_ptp as ""PTP"",jml_schedule as ""Schedule Call"",jml_left_msg as ""Left Message"",jml_nego as ""Negosiasi"",jml_busy as ""Busy"",jml_dead as ""Dead"",jml_invalid as ""Invalid"",jml_mailbox as ""Mailbox"",jml_pndah_alamat as ""Pindah Alamat"",jml_salbung as ""Salah Sambung"",jml_tdk_ditempat as ""Tidak Ada di Tempat"",jml_tdk_diangkat as ""Tidak Diangkat"",jml_unknow as ""Unknow"",jml_data_retur as ""Data Retur"",nominal_ptp as ""Nominal PTP"",tagihan as ""Tagihan"" from("
    strsql = strsql + vbCrLf + "  SELECT  date(tglcall) as tglcall, "
    strsql = strsql + vbCrLf + "  sum(CASE WHEN statuscall = 'Already Paid' then 1 else 0 end) as jml_paid,"
    strsql = strsql + vbCrLf + "  sum(CASE WHEN statuscall = 'BP' then 1 else 0 end) as jml_bp,"
