...
Public Function GETSPV(ByVal KetLevel As String) As Variant
    ...
    row = 1
    strsql = ""
    If KetLevel = "3" Then 'MANAGER
        row = 1
        For i = 1 To LVMgr.ListItems.Count
           If LVMgr.ListItems(i).Checked = True Then
                If MDIForm1.txtlevel.text <> "Administrator" Then
                    If MDIForm1.txtusername.text = LVMgr.ListItems(i).SubItems(1) Then
                        If row = 1 Then
    ...
    ElseIf KetLevel = "5" Then 'ADMIN
        ...
        For i = 1 To LVMgr.ListItems.Count
          ...
          End If
        Next i
    ElseIf KetLevel = "2" Then 'SUPERVISOR
        row = 1
...
InsertTable:
    ...
    If Level = "AGENT" Then
        ...
        sQuerySelect = " select A.id as id_cust,'New Data'::text,f_cek_new,'" & AGENT & "'::text,'" & nmAgent & "'::text,'" + MDIForm1.txtusername.text + "',now(),recsource from mgm a where  " & _
                       "  A.id in ( " & sQueryid & "  "
    Else
